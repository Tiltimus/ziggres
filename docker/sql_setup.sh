#!/bin/bash
set -e

cat > "$PGDATA/postgres.key" <<-EOKEY
-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
EOKEY
chmod 0600 "$PGDATA/postgres.key"

cat > "$PGDATA/postgres.crt" <<-EOCERT
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
EOCERT

cat >> "$PGDATA/postgresql.conf" <<-EOCONF
port = 5433
ssl = on
ssl_cert_file = 'postgres.crt'
ssl_key_file = 'postgres.key'
EOCONF

cat > "$PGDATA/pg_hba.conf" <<-EOCONF
# TYPE  DATABASE        USER            ADDRESS                 METHOD
host    all             pass_user       0.0.0.0/0            password
host    all             md5_user        0.0.0.0/0            md5
host    all             scram_user      0.0.0.0/0            scram-sha-256
host    all             pass_user       ::0/0                password
host    all             md5_user        ::0/0                md5
host    all             scram_user      ::0/0                scram-sha-256

hostssl all             ssl_user        0.0.0.0/0            trust
hostssl all             ssl_user        ::0/0                trust
host    all             ssl_user        0.0.0.0/0            reject
host    all             ssl_user        ::0/0                reject

hostssl all             ssl_scram_user  0.0.0.0/0            scram-sha-256
hostssl all             ssl_scram_user  ::0/0                scram-sha-256
host    all             ssl_scram_user  0.0.0.0/0            reject
host    all             ssl_scram_user  ::0/0                reject

# IPv4 local connections:
host    all             postgres        0.0.0.0/0            trust
# IPv6 local connections:
host    all             postgres        ::0/0                trust
# Unix socket connections:
local   all             postgres                             trust
EOCONF

psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" <<-EOSQL
    CREATE ROLE pass_user PASSWORD 'password' LOGIN;
    CREATE ROLE md5_user PASSWORD 'password' LOGIN;
    CREATE ROLE scram_user PASSWORD 'password' LOGIN;
    CREATE ROLE ssl_scram_user PASSWORD 'password' LOGIN;
    CREATE ROLE ssl_user LOGIN;
    CREATE EXTENSION hstore;
    CREATE EXTENSION citext;
    CREATE EXTENSION ltree;

    CREATE DATABASE ziggres;
    GRANT CONNECT ON DATABASE ziggres TO scram_user;
    GRANT ALL PRIVILEGES ON DATABASE ziggres TO scram_user;
EOSQL

psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname=ziggres <<-EOSQL
    GRANT pg_read_all_data TO scram_user;
    GRANT pg_write_all_data TO scram_user;
    GRANT ALL ON SCHEMA public TO scram_user;
EOSQL